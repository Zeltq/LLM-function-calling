...
В `.env` также можно переопределить другие параметры (см. `.env.example`):

```env
MODEL_ID=Qwen/Qwen2.5-3B-Instruct  # модель для загрузки
MAX_CONTEXT_MESSAGES=3              # глубина контекста (сообщений каждой роли)
MAX_NEW_TOKENS=256                  # лимит токенов при генерации
MAX_HISTORY_SIZE=1000               # макс. размер истории в интерактивном режиме
```
...
│   ├── test_parser.py    # Unit-тесты парсера (без загрузки модели)
│   └── runner.py         # Раннер + метрики
├── utils/
│   └── config.py         # Конфигурация: токен, модель, лимиты (из .env)
├── main.py               # Точка входа
├── requirements.txt
